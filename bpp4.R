...
bpp4<- function(age,heightz,gender,sysdia,bp=NULL,percentile=NULL) {
  # check that all parameters are provided and are valid
  if (any(!between(age,1,18),anyNA(c(bp,age,gender,heightz)))) return(NA)
  ...
  sum2<-sum(unlist(sapply(1:4,function(x) second(x,qnorm(heightz/100)))))
