...
abpmlms<-function(age=NULL,height=NULL,sysdia=sysdia,bp=NULL, gender,period,percentile=NULL) {
  ...
  cols<-c(1,2+1:3+(period-1)*3+(9*(sysdia-1)))
  if (!is.null(age)) {
  ...
  if (!is.null(bp)) {
    ...
    return(as.double(pnorm(as.numeric(z))))
  } else {
    result<-as.double(lms[2]*(1+lms[1]*lms[3]*qnorm(percentile))^(1/lms[1]))
    return (result)
  }
}
